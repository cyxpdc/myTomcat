...
public class HostConfig implements LifecycleListener, Runnable {
	 ...
	 * context name.
	 */
	private HashMap<String, Long> webXmlLastModified = new HashMap<>();
	...
	public void lifecycleEvent(LifecycleEvent event) {
		...
		try {
			...
			if (host instanceof StandardHost) {
				...
				//isDeployXML指明了Host实例是否要部署一个Context实例的描述符文件，默认下为true
				//liveDeploy指明了Host实例是否要周期性地检查一个新的部署
				//unpackWARs指明是要将WAR文件形式的Web应用程序解压缩
	...
	/**
	 * Deploy WAR files.
	 * 将位于%CATALINA_HOME/webapp%目录下的任何WAR文件进行部署
	 */
	protected void deployWARs(File appBase, String[] files) {
		for (int i = 0; i < files.length; i++) {
			...
			if (deployed.contains(files[i]))
				continue;
			File dir = new File(appBase, files[i]);
			if (files[i].toLowerCase().endsWith(".war")) {//以war为后缀
				deployed.add(files[i]);
				// Calculate the context path and make sure it is unique
	...
	/**
	 * Deploy directories.
	 * 可以将Web应用程序的整个目录复制到%CATALINA_HOME/webapps%目录下来完成Web应用程序的部署
	 */
