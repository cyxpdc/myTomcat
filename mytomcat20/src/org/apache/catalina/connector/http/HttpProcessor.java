...
final class HttpProcessor implements Lifecycle, Runnable {
	 ...
	 *
	 * @param connector
	 *            HttpConnector that owns this processor
	 * @param id
	 *            Identifier of this HttpProcessor (unique per connector)
	 */
	...
	/**
	 * The socket we are currently processing a request for. This object is used
	 * for inter-thread communication only.
	 */
	 ...
	 * has successfully parsed the request headers, and before starting reading
	 ...
	 * <b>NOTE</b>: This method is called from our Connector's thread. We must
	 * assign it to our own thread so that multiple simultaneous requests can be
	 * handled.
	 *
	 * @param socket
	...
	/**
	 * Await a newly assigned Socket from our Connector, or <code>null</code> if
	 ...
	 *
	 * @param value
	 *            The value of the <code>Accept-Language</code> header.
	 */
	 ...
	 *
	 * @param socket
	 *            The socket on which we are connected
	 *
	 ...
	 *
	 * @param input
	 *            The input stream attached to our socket
	 * @param output
	 ...
	 * 发送如此格式的字符串：HTTP/1.1 100 Continue\r\n\r\n
	 * @param output
	...
	private void process(Socket socket) {
		boolean ok = true;//表示处理过程中是否有错误发生
		...
		try {
			input = new SocketInputStream
					(socket.getInputStream(), connector.getBufferSize());
		} catch (Exception e) {
			log("process.create", e);
			ok = false;
		}
		//stopped：HttpProcessor实例是否被连接器终止，是的话，process也应该停止
		//http11表明请求是否是从支持HTTP1.1的客户端发送来的
		keepAlive = true;//持久连接
		//不断地读取输入流，直到HttpProcessor实例终止，抛出一个异常，或连接断开
		//stopped会在threadStop变为true
		while (!stopped && ok && keepAlive) {
			//获取输出流对象，执行request和response的一些初始化操作
			finishResponse = true;
			try {
			...
			//解析Request请求：parseConnection、parseRequest、parseHeaders
			try {
				if (ok) {
			...
			//将request和response对象作为参数传入servlet容器的invoke方法
			try {
				((HttpServletResponse) response).
				setHeader("Date", FastHttpDateFormat.getCurrentDate());
				if (ok) {
			...
			//如果finishResponse为true，则调用finishResponse和finishRequest
			//再将结果发送至客户端
			if (finishResponse) {
				try {
				...
				try {
					if (output != null)
						output.flush();//将结果发送至客户端
				} catch (IOException e) {
					ok = false;
				}
			}
			//检查response的头信息"Connection"在servelt中是否被设置为"close"
			//或协议为HTTP1.0
			//若任意一种为真，则将keepAlive设置为false
			if ("close".equals(response.getHeader("Connection"))) {
				keepAlive = false;
			}
			// End of request processing
			status = Constants.PROCESSOR_IDLE;
			//回收request和response对象,即属性都赋值为null
			request.recycle();
			response.recycle();
		}
		//若keepAlive为true，或前面的解析工作没有发生错误，或HttpProcessor对象没有被终止，则继续while
		//否则while退出，调用shutdownInput和关闭socket
		try {
			shutdownInput(input);//检查是否有未读完的字节，若有，则跳过这些字节
			socket.close();
		} catch (IOException e) {
			e.printStackTrace();
		} catch (Throwable e) {
	...
	public void run() {
		...
		while (!stopped) {
			//阻塞，直到获取Socket，即直到HttpConnector调用HttpProcessor的assign方法
			Socket socket = await();
			if (socket == null)
				continue;
			//处理请求
	 ...
	 */
	public void ru() throws LifecycleException {
		if (started)
