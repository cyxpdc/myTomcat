...
public abstract class PersistentManagerBase extends ManagerBase implements Lifecycle, PropertyChangeListener, Runnable {
	...
	public void processPersistenceChecks() {
		processMaxIdleSwaps();//当某个Session对象闲置了过长事件时，根据条件换出
		processMaxActiveSwaps();//内存中有过多的Session对象时换出
		processMaxIdleBackups();//对该Session对象进行备份
	}
		...
		for (int i = 0; i < sessions.length && toswap > 0; i++) {
			...
			if (timeIdle > minIdleSwap) {
				...
				try {
					...
					e.printStackTrace();; // This is logged in writeSession()
				}
				toswap--;
			}
		}
