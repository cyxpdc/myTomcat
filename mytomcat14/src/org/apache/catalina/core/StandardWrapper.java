...
public final class StandardWrapper extends ContainerBase implements ServletConfig, Wrapper {
	...
	public Servlet allocate() throws ServletException {
		...
		synchronized (instancePool) {
			while (countAllocated >= nInstances) {//如果已分配的数量比当前STM servlet数量少，那么就可以直接分配
				...
				if (nInstances < maxInstances) {
					...
					try {
						instancePool.wait();//等待，直到某个STM servlet实例被放回到栈中
	...
	public synchronized Servlet loadServlet() throws ServletException {
		...
		try {
			...
			if ((servlet instanceof ContainerServlet) && isContainerProvidedServlet(actualClass)) {
				...
				((ContainerServlet) servlet).setWrapper(this);//可以访问Catalina内部功能
