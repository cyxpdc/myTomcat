...
public final class StandardService implements Lifecycle, Service {
	...
	public void removeConnector(Connector connector) {
		synchronized (connectors) {
			...
			if (removeConnector < 0)
				return;
			if (started && (connectors[removeConnector] instanceof Lifecycle)) {
				try {
					((Lifecycle) connectors[removeConnector]).stop();
				} catch (LifecycleException e) {
					;
				}
			}
			connectors[removeConnector].setContainer(null);
			connector.setService(null);
			int k = 0;
			Connector results[] = new Connector[connectors.length - 1];
			for (int i = 0; i < connectors.length; i++) {
				if (i != removeConnector)
