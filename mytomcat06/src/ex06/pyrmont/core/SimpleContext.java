...
public class SimpleContext implements Context, Pipeline, Lifecycle {
	...
	/**
	 * URL:Wrapper context.addServletMapping("/Primitive", "Primitive");
	...
	public synchronized void start() throws LifecycleException {
		if (started)//如果已经启动过，则抛异常
			throw new LifecycleException("SimpleContext has already started");
		// 触发BEFORE_START_EVENT事件
		lifecycle.fireLifecycleEvent(BEFORE_START_EVENT, null);
		started = true;//该组件已启动
		try {//启动组件(管道、类加载器)和子容器(两个SimpleWrapper)
	...
	public void stop() throws LifecycleException {
		...
		//触发BEFORE_STOP_EVENT和STOP_EVENT事件
		lifecycle.fireLifecycleEvent(BEFORE_STOP_EVENT, null);
		lifecycle.fireLifecycleEvent(STOP_EVENT, null);
		started = false;
		try {
			//关闭组件(管道、类加载器)和子容器(两个SimpleWrapper)
			if (pipeline instanceof Lifecycle) {
